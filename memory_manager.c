...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size) {
	for(int i=0; i<size/PAGE_SIZE;i++)
		...
		struct WorkingSetElement*FindElementInA;
		// SEARCH IN ACTIVE
		LIST_FOREACH(FindElementInA,&(e->ActiveList))
		{
			uint32 va=FindElementInA->virtual_address;
			// IF IN RANGE
			if(va>=virtual_address&&va<(virtual_address+size)){
				uint32*pagetable=NULL;
				struct Frame_Info*f=get_frame_info(e->env_page_directory,(void*)va,&pagetable);
				unmap_frame(e->env_page_directory,(void*)va);
				LIST_REMOVE(&(e->ActiveList),FindElementInA);
				FindElementInA->empty=1;
				// Add to workingset
				LIST_INSERT_HEAD(&(e->PageWorkingSetList),FindElementInA);
				// IF CONTAIN ELEMENTS
				...
				if(LIST_SIZE(&(e->SecondList))!=0){
					...
					pt_set_page_permissions(e,l_in_sec->virtual_address,PERM_PRESENT|PERM_WRITEABLE,0);
				}
                 // Not exist in memory
				pt_clear_page_table_entry(e,FindElementInA->virtual_address);
		...
		struct WorkingSetElement *findinsec;
		// IF FOUNDED IN SECOND
