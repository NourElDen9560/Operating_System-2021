...
void page_fault_handler(struct Env * curenv, uint32 fault_va) {
	...
	int A_Size = LIST_SIZE(&(curenv->ActiveList)) ;
	// If the Active list is not Full
	if (curenv->ActiveListSize >A_Size) {
		Placement(curenv, fault_va);
	}
	// The ActiveList is Full and the second is not Full
	else if (curenv->ActiveListSize == curenv->ActiveList.size
			&& curenv->SecondListSize != curenv->SecondList.size) {
		bool foundedInSecond = 0;
		// Remove element form active and put it in second with perm 0
		LIST_REMOVE(&curenv->ActiveList, last_in_active);
		LIST_INSERT_HEAD(&curenv->SecondList, last_in_active);
		pt_set_page_permissions(curenv, last_in_active->virtual_address, 0,
				PERM_PRESENT);
		if (LIST_SIZE(&(curenv->SecondList)) != 0) {
			struct WorkingSetElement *found_in_second;
			// Search to check if it found in second or not
			...
			if (foundedInSecond) {
				...
				found_in_second->virtual_address, PERM_PRESENT, 0);
	...
	// Replacement
		...
		struct WorkingSetElement* last_in_active = LIST_LAST(
		&curenv->ActiveList);
		LIST_REMOVE(&curenv->ActiveList, last_in_active);
		pt_set_page_permissions(curenv, last_in_active->virtual_address, 0,
				PERM_PRESENT);
		...
		if (foundedInSecond) {
			LIST_REMOVE(&curenv->SecondList, found_in_second);
			pt_set_page_permissions(curenv, found_in_second->virtual_address,
					PERM_PRESENT, 0);
			LIST_INSERT_HEAD(&curenv->ActiveList, found_in_second);
			return;
		}
		// victim to remove from second
		struct WorkingSetElement* victim = LIST_LAST(
				&curenv->SecondList);
...
// IF MODIFIED IN MEMORY
		if ((page_permissions & PERM_MODIFIED)) {
			...
			struct Frame_Info*ptr = get_frame_info(curenv->env_page_directory,
					(void*) (victim->virtual_address), &p_t);
			pf_update_env_page(curenv,
					(void*) (victim->virtual_address), ptr);
		}
...
void Placement(struct Env * curenv, uint32 fault_va) {
	//  Allocate the frame and map
	uint32 *ptr_p_t = NULL;
	struct Frame_Info *p_a = get_frame_info(curenv->env_page_directory,
			(void*) fault_va, &ptr_p_t);
	int ret = allocate_frame(&p_a);
	if (ret != E_NO_MEM) {

		map_frame(curenv->env_page_directory, p_a, (void*) fault_va,
				PERM_WRITEABLE | PERM_PRESENT | PERM_USER);
	}
	struct WorkingSetElement* ele = LIST_FIRST(&(curenv->PageWorkingSetList));
	ele->virtual_address = fault_va;
	// After Mapping
	// We remove the element from Working set and insert it in the head of the active
	LIST_REMOVE(&curenv->PageWorkingSetList, ele);
