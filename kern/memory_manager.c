...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size) {
	int counter = ROUNDUP(size, PAGE_SIZE) / PAGE_SIZE;
...
// [2] freeMem
void DeleteInActive(struct Env *e, struct WorkingSetElement *findinActive,
		uint32*pagetable);
void DeleteInSecond(struct Env *e, struct WorkingSetElement *findinsec,
		uint32*pagetable);
void freeMem(struct Env* e, uint32 virtual_address, uint32 size) {
	for (int i = 0; i < size / PAGE_SIZE; i++) {
		uint32 add = virtual_address + (i * PAGE_SIZE);
		pf_remove_env_page(e, add);
	//	pf_remove_env_page(e , virtual_address + (i*PAGE_SIZE))
	}
	struct WorkingSetElement*findinActive, *findinsec;
	LIST_FOREACH(findinActive,&(e->ActiveList))
	{
		uint32 va = findinActive->virtual_address;
		//IF IN RANGE ?
		if (va >= virtual_address && va < (virtual_address + size)) {
			uint32*pagetable = NULL;
			struct Frame_Info*f = get_frame_info(e->env_page_directory,
					(void*) findinActive->virtual_address, &pagetable);
			unmap_frame(e->env_page_directory, (void*) va);
			DeleteInActive(e, findinActive, pagetable);
		}

	}
	LIST_FOREACH(findinsec,&(e->SecondList))
	{
		uint32 va = findinsec->virtual_address;
		if (va >= virtual_address && va < (virtual_address + size)) {
			uint32*pagetable = NULL;
			struct Frame_Info*f = get_frame_info(e->env_page_directory,
					(void*) findinsec->virtual_address, &pagetable);
			unmap_frame(e->env_page_directory, (void*) virtual_address);
			DeleteInSecond(e, findinsec, pagetable);
		}
	}

}

// DELETE IN ACTIVE AND SECOND
void DeleteInActive(struct Env *e, struct WorkingSetElement *findinActive,
		uint32*pagetable) {
	uint32 va = findinActive->virtual_address;
	LIST_REMOVE(&(e->ActiveList), findinActive);
	findinActive->empty = 1;
	// Add to workingset
	LIST_INSERT_HEAD(&(e->PageWorkingSetList), findinActive);
	// IF CONTAIN ELEMENTS
int SecondSize = LIST_SIZE(&(e->SecondList)) ;
	if (SecondSize != 0) {
	...
	// Not exist in memory
	pt_clear_page_table_entry(e, findinActive->virtual_address);
...
void DeleteInSecond(struct Env *e, struct WorkingSetElement *findinsec,
		uint32 *pagetable) {
	uint32 va = findinsec->virtual_address;
	LIST_REMOVE(&(e->SecondList), findinsec);
	findinsec->empty = 1;
	LIST_INSERT_HEAD(&(e->PageWorkingSetList), findinsec);
	pt_clear_page_table_entry(e, findinsec->virtual_address);
	for (int j = 0; j < 1024; j++) {
		if (pagetable[j] != 0)
			break;
		else {
			if (j == 1023) {
				free_frame(to_frame_info((uint32) pagetable));
				pd_set_table_unused(e, va);
				pd_clear_page_dir_entry(e, va);
			}
		}
